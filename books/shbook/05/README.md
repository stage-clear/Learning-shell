...
- `trap` コマンドは、だいたいがシェルスクリプトの中で、シグナルを受け取ったときにできるだけ
後処理をやってから終了しようとする場合に利用します。そういう目的であれば普通、`1` `2` `3` `15`
を捕まえるようにします
...
シングルクォートで囲むのは、`trap` コマンドに処理を渡す前にメタキャラクタや変数を展開して
しまうことを防ぐためです。


### 5.4.2 シグナルの無視

- コマンドのところに何も書かない（空のコマンドを指定する）と、そのあとに書いたシグナルを
受け取っても無視します。シグナルを受けていないように平然と処理を続けます。
...
- `trap` の `command` 部分に何も書かなかった場合、シグナルを受け取った時に何も行わない
のではなく、そのシグナルの本来の意味のままに動作します
...
```sh
trap '' 2 3     # 2番と3番を無視させる
trap ':' 2      # 2番（割り込み）を受けるようにする
cat file        # cat コマンドの間だけは、割り込みキーが有効になる
trap '' 2       # 2番を無視させる
```

無限ループを抜けるためにキーボードからの割り込みを使う
...
- リモートシェルの場合そのコマンドの名前が統一されきっていないということです
普通は `rsh` という名称で、HP-UX では `remsh` 、SCO UNIX の場合は `rcmd` という名を使っています


__例) リモートコマンドが使えるかどうかのテスト__

これを実行して、うまくいかない限り、他のどんなリモートコマンドも正しく処理できるものではないでしょう

```sh
rsh host "echo testing > /dev/null"
if [ $? -ne 0 ]; then
